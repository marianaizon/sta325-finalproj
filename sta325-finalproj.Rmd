...
```{r log-bc-trans}
min(train$ARR_DELAY, na.rm = TRUE)

train$adj_ARR_DELAY <- train$ARR_DELAY + 75
min(train$adj_ARR_DELAY, na.rm = T)

adj_log_linear_model <- lm(adj_ARR_DELAY ~ DEP_DELAY + OP_CARRIER + DEST + CRS_DEP_TIME  + log_TAXI_OUT + log_TAXI_IN + TYPE_DELAY + OP_CARRIER:DEST + DEST:log_TAXI_IN+log_TAXI_OUT:DEP_DELAY, train)

library(EnvStats)
bc_model <- boxcox(adj_log_linear_model, optimize = TRUE)
bc_lambda <- bc_model$lambda
bc_lambda
plot(bc_model)

# library(car)
# hello <- boxCox(log_linear_model, family="yjPower", plotit = T)
# hello
```
...
```{r, message=FALSE}
library(MASS)
step_model <- stepAIC(full_model2, trace = FALSE)
summary(step_model)
```
...
yhat.boost <- predict(boost.delay, newdata =test,
                       n.trees = 10000)
boost_MSE <- sum((test$ARR_DELAY-yhat.boost)^2)/328
boost_MSE #returning NA for some reason...
```
